...
use talpid_core::proxy::{self, ProxyMonitor, ProxyResourceData};
#[cfg(target_os = "linux")]
use talpid_core::routing::{self, RequiredRoute};
...
impl WintunContext for WintunContextImpl {
    ...
    async fn wait_for_interfaces(&self) -> io::Result<()> {
        ...
        talpid_core::windows::wait_for_interfaces(luid, true, self.wait_v6_interface).await
    }
